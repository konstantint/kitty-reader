...
function localSyllabifyAndFormat(text: string): string {
    // Fix for 'ё' which is incorrectly handled by JS .toUpperCase()
    const upperCaseText = text.replace(/ё/g, 'Ё').toUpperCase();
    ...
    return parts.map(part => {
        ...
        // Separate word from trailing punctuation, now including 'Ё'
        const match = part.match(/^([A-ZА-ЯÄÖÜЁ]+)([^A-ZА-ЯÄÖÜЁ]*)$/);
        if (match) {
 ...
 */
function syllabifyText(text: string): ProcessedText {
